...
#include <QRandomGenerator>        // Not used anywhere → can be removed
...
// They prevent the use of “magic numbers” (0, 1, 2, 3) throughout
...
//   Tracks whether the user is practicing English→Spanish (true)
//   or Spanish→English (false). Many parts of the program behave
...
// -----------------------------------------------------------
// MEMORY SYSTEM — where saved word fragments are stored,
...
//   parses the JSON, and loads all saved Spanish→English
...
//
// addToMemory(es, en)
//   Saves a Spanish→English pair:
//     • Updates the existing entry if Spanish already exists.
...
// g_memory:
//   Stores all saved Spanish→English fragments the user has learned.
...
// saves or updates a Spanish→English pair.
      ...
      ui(new Ui::MainWindow)
{
    ...
    // ===============
    // Text explaining workflow: Add → AI split → Submit.
    // ===============
    ui->lblInstructions->setText(
        "1. Enter a sentence in Spanish and its English translation.\n"
        "2. Click Add.\n"
    ...
    // Placeholders — Spanish placeholder in English
    ...
    connect(ui->btnAddRow, &QPushButton::clicked, this, [=]() {
        ...
        // In EN→SP mode, Spanish is intentionally empty.
        // Only require Spanish sentence when NOT in English→Spanish mode.
        ...
        if (englishToSpanishMode)
        {
            // EN → SP mode
            prompt = QString(
                "You are a Spanish–English grammar processor.\n"
                ...
                "\n"
                "RULES:\n"
                "  • First translate the full English sentence into natural Spanish.\n"
                "  • Split BOTH languages into matching grammatical elements.\n"
                "  • Spanish elements must be contiguous substrings of the Spanish translation.\n"
                "  • English elements must be contiguous substrings of the original English sentence.\n"
                "  • NO explanations.\n"
                "  • NO markdown.\n"
        ...
        else
        {
            ...
            prompt = QString(
                ...
                "\n"
                "RULES:\n"
                "  • FullEnglish must be one complete natural English translation.\n"
                "  • Split both languages into matching grammatical elements.\n"
                "  • Spanish must be contiguous substrings.\n"
                "  • English must be contiguous substrings of the translation.\n"
                "  • NO explanations.\n"
                "  • NO markdown.\n"
                "  • Output ONLY valid JSON.\n"
                "\n"
        ...
        connect(reply, &QNetworkReply::finished, this, [=]() {
            ...
            QJsonObject jsonObj = parsed.object();
            QString fullEnglish = jsonObj.value("FullEnglish").toString().trimmed();



            QJsonArray elems = jsonObj.value("Elements").toArray();
            // In EN→SP mode, DO NOT clear bottom sentence — user must still see it.
            // Only clear English in SP→EN mode.
            if (!englishToSpanishMode)
            ...
            for (int i = 0; i < elems.size(); ++i)
            {
                ...
                esEdit->setFixedHeight(40);   // <<— INCREASED HEIGHT
                ...
                enEdit->setFixedHeight(40);    // <<— REDUCED HEIGHT
                ...
                // ===========================
                QPushButton *audioBtn = new QPushButton("🔊");
    ...
    connect(ui->btnRandomSpanish, &QPushButton::clicked, this, [=]() {
        ...
        // Minimal natural generator — no “use as many words as possible”
        QString prompt = QString(
            "You are generating a NATURAL Spanish sentence using ONLY the following fragments:\n"
            "%1\n\n"
            "RULES:\n"
            "  • Use only the fragments — but you do NOT need to use all of them.\n"
            "  • Choose whichever fragments form ONE natural, grammatically correct sentence.\n"
            "  • You MAY reorder fragments.\n"
            "  • You MAY add minimal connectors (y, pero, que, para, a, de).\n"
            "  • You may NOT invent new nouns or verbs.\n"
            "  • Output ONLY JSON:\n"
            "%2\n"
...
void MainWindow::onSubmitClicked()
{
    ...
    for (int r = 0; r < rows; ++r)
    {
        ...
        if (enToSp)
        {
            ...
            // English is always a clue → always green
    ...
    // =========================
    // NEVER rebuild in EN→SP (breaks logic!)
    if (!enToSp)
    {
    ...
    // =========================
    // EN→SP: BUILD SPANISH FULL SENTENCE MANUALLY
    // =========================
    if (enToSp)
    {
        QStringList sp;
        ...
        // English full sentence stays — do not touch it.
    }
}
...
void MainWindow::loadBooksList()
{
    ...
    ui->comboBooks->addItem("➕ Add new book...");
}
...
{
    //
    // STEP 1 — Ask user for TXT/JSON file
    //
    ...
    //
    // STEP 3 — Parse file into clean sentence list
    //
    ...
    if (srcPath.endsWith(".txt", Qt::CaseInsensitive))
    {
        ...
        if (!f.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            QMessageBox::warning(this, "Error",
                                 "Cannot open the selected TXT file.");
            return "";
        }

        QString whole = QString::fromUtf8(f.readAll());
        f.close();
        ...
        // Clean and append each sentence
        for (QString s : list)
        {
            s = s.trimmed();
            if (!s.isEmpty())
    ...
    //
    // STEP 5 — Write JSON book to internal storage
    //
    QJsonObject outObj;
    ...
    //
    // STEP 6 — Refresh dropdown + confirm
    //
    loadBooksList();
...
void MainWindow::loadBookNextSentence()
{
    ...
    ui->txtEnglishFull->clear();
    englishToSpanishMode = false;   // <-- FORCE SP→EN mode for book sentences
...
void MainWindow::restartCurrentBook()
{
    ...
    if (book.isEmpty() || book == "➕ Add new book...")
    {
